...
const request12306 = function doReq12306(frm, to, queryDate, purposeCode) {
  ...
  const url = `https://kyfw.12306.cn/otn/lcxxcx/query?purpose_codes=${purposeCode}&queryDate=${queryDate}&from_station=${frm}&to_station=${to}`;
  const options = {
    url,
    headers: {
      'User-Agent': 'request'
...
const query = function doQuery(frm, to, at) {
  ...
  if (!frmCode || !toCode) {
    ...
    request12306(frmCode[1], toCode[1], at);
  }
};
